...
from gpudrive.cl4ad import DefaultCurriculum, DomainRandomization, PrioritizedLevelReplay
...
def download_wandb_run_file(filename: str, run_path: str):
    api = wandb.Api()
    ...
    if file_to_download is None:
        raise ValueError(f"{filename} is not found in {run_path}")
    return file_to_download.download(exist_ok=True).name
...
def make_agent(env, config):
    ...
    if config.continue_training:
        ...
        # Load checkpoint
        if config.wandb_run_to_restore is not None:
            model_cpt_path = download_wandb_run_file(filename=config.model_cpt,
                                                     run_path=config.wandb_run_to_restore)
        else:
            model_cpt_path = config.model_cpt
...
def train(args, vecenv):
    ...
    if args.continue_training:
        ...
        if args.wandb_run_to_restore is not None:
            curriculum_cpt_path = download_wandb_run_file(filename=args.curriculum_cpt,
                                                          run_path=args.wandb_run_to_restore)
        else:
            curriculum_cpt_path = args.curriculum_cpt
        vecenv.curriculum.load(curriculum_cpt_path)
...
def run(
    ...
    ] = "baselines/ppo/config/ppo_base_puffer_cl4ad_plr.yaml",
    *,
    # fmt: off
    # data options
    data_dir: Annotated[Optional[str], typer.Option(help="The path to the data directory")] = None,
    # Continue training options
    continue_training: Annotated[Optional[bool], typer.Option(help="Continue training")] = None,
    model_cpt: Annotated[Optional[str], typer.Option(help="Model checkpoint to restart training with")] = None,
    curriculum_cpt: Annotated[Optional[str], typer.Option(help="Curriculum checkpoint to restart training with")] = None,
    wandb_run_to_restore: Annotated[Optional[str], typer.Option(help="Wandb run name")] = None,
    # Environment options
    ...
    curriculum_type: Annotated[Optional[str], typer.Option(help="Curriculum type; 'default' or 'domain_randomization'")] = None,
    # PLR options
    lambd: Annotated[Optional[float], typer.Option(help="Lambda for PLR")] = None,
    buffer_size: Annotated[Optional[int], typer.Option(help="Buffer size for PLR")] = None,
    replay_rate: Annotated[Optional[float], typer.Option(help="Replay rate for PLR")] = None,
    beta: Annotated[Optional[float], typer.Option(help="Beta for PLR")] = None,
    rho: Annotated[Optional[float], typer.Option(help="Rho for PLR")] = None,
    score_type: Annotated[Optional[str], typer.Option(
        help="Score type for PLR; 'amgae', 'pvl', 'maxmc', 'learn', 'learn-goal', 'gc-ade', 'act-mae'")] = None,
    warm_up_steps: Annotated[Optional[int], typer.Option(help="Warm-up steps for training")] = None,
    ...
    # Continue training options
    continue_training_config = {
        "continue_training": continue_training,
        "model_cpt": model_cpt,
        "curriculum_cpt": curriculum_cpt,
        "wandb_run_to_restore": wandb_run_to_restore,
    }
    config.__dict__.update(
        {k: v for k, v in continue_training_config.items() if v is not None}
    )
    ...
    config.environment.__dict__.update(
    ...
    curriculum = None
    if config.train.curriculum_type == "default":
        print("Resample scenes: ", config.train.resample_scenes)
        curriculum = DefaultCurriculum(data_loader=train_loader)
    elif config.train.curriculum_type == "domain_randomization":
        curriculum = DomainRandomization(data_loader=train_loader)
    elif config.train.curriculum_type == "plr":
        curriculum = PrioritizedLevelReplay(
            data_loader=train_loader,
            gamma=config.train.gamma,
            lambd=config.train.lambd,
            buffer_size=config.train.buffer_size,
            replay_rate=config.train.replay_rate,
            beta=config.train.beta,
            rho=config.train.rho,
            score_type=config.train.score_type,
            warm_up_steps=config.train.warm_up_steps,
        )
    else:
        raise ValueError(
            f"Unknown curriculum type: {config.train.curriculum_type}. "
            "Choose 'default' or 'domain_randomization'."
        )
