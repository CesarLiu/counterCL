...
if __name__ == "__main__":
    ...
    MODEL_TO_LOAD = "daphne-cornelisse/policy_S1000_02_27"
    filename_postfix = "_deterministic"
    DATASET = "test"
    SORT_BY = None #"goal_achieved_frac" 
    SHOW_TOP_K = 6 # 50 # Render this many scenes

    # Configurations
    eval_config = load_config("examples/experimental/config/eval_config_huggingface")
    model_config = load_config("examples/experimental/config/model_config_huggingface")
    ...
    sim_state_frames, filenames = make_videos(
        ...
        dataset=DATASET,
        device=eval_config.device,
        zoom_radius=eval_config.obs_radius, # 40,
    ...
    for env_id, frames in sim_state_arrays.items():
        ...
        if df_res:
            ...
            video_path = videos_dir / f"{filename}_ga_{goal_achieved:.2f}__cr_{collided:.2f}__or_{off_road:.2f}__ot_{other:.2f}{filename_postfix}.gif"
        else:
            video_path = videos_dir / f"{filename}{filename_postfix}.gif"
