...
def evaluate(data):
    ...
    if(data.resample_buffer >= data.config.resample_interval
       and ((data.config.resample_scenes
       and data.config.resample_dataset_size > data.vecenv.num_worlds)
           or str(data.vecenv.curriculum) != "default")
       ):
        ...
        with profile.eval_misc:
            ...
            for i in info:
                for k, v in pufferlib.utils.unroll_nested_dict(i):
                    if "curriculum" in k:
                        curriculum_infos[k[len("curriculum/"):]] = v
                    else:
                        data.infos[k].append(v)
            
            # Update curriculum
            if len(curriculum_infos) > 0:
                curriculum_infos.update({"values": value.cpu().numpy()})
                data.vecenv.curriculum.update(
                    infos = curriculum_infos,
                )
...
class Experience:
    ...
    def sort_training_data(self):
        # Sorting with respect to agent IDs (env_id) and step
        idxs = np.asarray(
...
def save_checkpoint(data, save_checkpoint_to_wandb=True):
    ...
    curriculum_path = os.path.join(path, f"curriculum_{config.exp_id}_{data.epoch:06d}.pkl")
    data.vecenv.curriculum.save(curriculum_path)
    if save_checkpoint_to_wandb and data.wandb is not None:
        ...
        data.wandb.save(model_path)
        data.wandb.save(curriculum_path)
    return model_path
