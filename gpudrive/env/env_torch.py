...
class GPUDriveTorchEnv(GPUDriveGymEnv):
    ...
    def get_distance_to_log_trajectory(self, world_time_steps):
        log_trajectory = LogTrajectory.from_tensor(
            self.sim.expert_trajectory_tensor(),
            self.num_worlds,
            self.max_agent_count,
        ...
        # return square of the euclidean distance between agent and logs
        return torch.norm(
            log_traj_pos_tensor - agent_pos, dim=-1
            )**2

    def get_distance_to_log_actions(self, actions, world_time_steps):
        """Calculate the distance between the provided actions and the logged actions."""
        if (
            self.config.dynamics_model == "classic"
            or self.config.dynamics_model == "bicycle"
            or self.config.dynamics_model == "delta_local"
        ...
        if getattr(self, "log_playback_traj", None) is None:
            self.log_playback_traj, _, _, _ = self.get_expert_actions()

        log_action = []
        for i in range(self.num_worlds):
            log_action.append(
                self.log_playback_traj[i, :, world_time_steps[i], :]
            )
        log_action_tensor = torch.stack(log_action)

        # return square of the euclidean distance between agent and logs
        return torch.norm(
            log_action_tensor - action_value_tensor, p=1, dim=-1
            )
