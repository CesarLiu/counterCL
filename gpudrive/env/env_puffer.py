...
from gpudrive.cl4ad import DefaultCurriculum
...
class PufferGPUDrive(PufferEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if curriculum is None:
            curriculum = DefaultCurriculum(data_loader)
        ...
        self.single_observation_space = self.env.single_observation_space
         ...
         # Sample initial batch of scenarios
        self.env.swap_data_batch(self.curriculum.sample())
        ...
        if self.num_agents == 0:
            print("Number of agents is zero. Data batch:")
            print(self.env.data_batch)
        num_controlled_agents_per_scene = self.controlled_agent_mask.sum(dim=1).cpu().numpy()
        scenes_without_controlled_agents = np.where(num_controlled_agents_per_scene == 0)[0]
        if scenes_without_controlled_agents.shape[0] != 0:
            for s in scenes_without_controlled_agents:
                print(f"Scene without controlled agents: {self.env.data_batch[s]}")
        ...
        info_lst = [
            {
                "curriculum":
                {
                    ...
                    "collision": self.collided_in_episode[self.controlled_agent_mask].cpu().numpy().astype(np.int32),
                    "off-road": self.offroad_in_episode[self.controlled_agent_mask].cpu().numpy().astype(np.int32),
                    "d2lt": self.env.get_distance_to_log_trajectory(
                        world_time_steps=self.episode_lengths[:, 0].long()-1
                        )[self.controlled_agent_mask].cpu().numpy(),
                    "d2la": self.env.get_distance_to_log_actions(
                        actions=self.actions, 
                        world_time_steps=self.episode_lengths[:, 0].long()-1
                        )[self.controlled_agent_mask].cpu().numpy(),
                    "done_worlds": done_worlds_cpu,
                }
            }
        ]
    ...
    def resample_scenario_batch(self):
        ...
        # Swap the data batch by sampling from the curriculum
        self.env.swap_data_batch(self.curriculum.sample())
    ...
    def log_data_coverage(self):
        ...
        # Get unique scenarios in the dataset, and min id in dataset to account for repetitions
        unique_scenes_in_dataset = sorted(list({
            file: idx for idx, file in enumerate(self.env.data_loader.dataset)
        }.values()))
        min_id_in_dataset = min(unique_scenes_in_dataset)

        # Get the counts of scenarios in the current batch
        scenario_counts = [self.file_to_index[file] for file in self.env.data_batch]
        scenario_unique = len(set(self.env.data_batch))
        hist_bins = list(range(min_id_in_dataset, min_id_in_dataset+len(unique_scenes_in_dataset)+1))
        if len(hist_bins) > 512:
            hist_bins = np.linspace(min_id_in_dataset, 
            min_id_in_dataset+len(unique_scenes_in_dataset), num=512).tolist()
